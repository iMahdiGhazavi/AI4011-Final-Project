...
class approximateQlearning(Qlearning):
    ...
    def __init__(self, niterations) -> None:
        self.weights = [1 , 1, 1, 1, 1, 1, 1, 1 , 1]
        # self.weights = load_approximate_weights()
        Qlearning.__init__(self, niterations)
...
class DeepQ:
    ...
    def __init__(self , statesize , actsize , niteration):
    ...
    def buildnetwork(self):
        model = Sequential()
        model.add(layers.Dense(8 , input_dim=self.statesize , activation='relu'))
        model.add(layers.Dense(16 , activation='relu'))
        model.add(layers.Dense(self.actsize , activation='linear'))
    ...
    def getAction(self, state , iternumber):
        self.setparametr(iternumber)
        if flipcoin(self.epsilon):
            action = random.randrange(self.actsize)
        else:
    ...
    def update(self, batch_size):
        ...
        for state, action, reward, next_state, done in minibatch:
            if not done:
                targets = reward + self.gamma * np.amax(self.model.predict(next_state)[0])
            else:
                targets = reward  
    ...
    def train(self , env):
        ...
        for i in range(self.niteration):
            while True:
                currentstate = np.reshape(nextstate , [1 , self.statesize])
                action = self.getAction(currentstate , i)
                nextstate, reward, terminated, truncated, info = env.step(action)
                nextstatetmp = np.reshape(nextstate , [1 , self.statesize])
                self.memorize(currentstate , action , reward , nextstatetmp , terminated or truncated)
                totalreward += reward
                env.render()
                if terminated or truncated:
                    print(totalreward)
